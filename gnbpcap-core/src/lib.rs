//! Core PCAP-parsing logic for gnbpcap, extracted so it can be shared
//! between the Tauri desktop app (src-tauri) and the MCP-facing CLI
//! (gnbpcap-cli), rather than duplicated. No Tauri types live here —
//! everything is plain, synchronous, and safe to call from any context.
...
pub struct ParseResult {
    ...
    pub selected_decode_opts: Vec<String>,
...
pub fn find_tshark() -> Option<String> {
...
pub fn parse_pcap_blocking(file_path: &str) -> Result<ParseResult, String> {
...
pub fn get_packet_details_blocking(
    file_path: &str,
    frame_num: u32,
