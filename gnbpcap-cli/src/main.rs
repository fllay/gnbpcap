...
fn main() {
    ...
    let result = match args.get(1).map(String::as_str) {
        ...
        Some("details") => run_details(&args[2..]),
        _ => {
            eprintln!(
                "usage:\n  gnbpcap-cli parse <file.pcap>\n  gnbpcap-cli details <file.pcap> <frame_num> [--decode-opt <opt>]..."
            );
            std::process::exit(2);
        }
    };
...
fn run_parse(args: &[String]) -> Result<(), String> {
    let file_path = args.get(0).ok_or("missing <file.pcap> argument")?;
    let result = parse_pcap_blocking(file_path)?;
    println!("{}", serde_json::to_string(&result).map_err(|e| e.to_string())?);
    Ok(())
}

fn run_details(args: &[String]) -> Result<(), String> {
    let file_path = args.get(0).ok_or("missing <file.pcap> argument")?;
    let frame_num: u32 = args
        .get(1)
        .ok_or("missing <frame_num> argument")?
        .parse()
        .map_err(|_| "frame_num must be an integer".to_string())?;

    let mut decode_opts = Vec::new();
    let mut i = 2;
    while i < args.len() {
        if args[i] == "--decode-opt" {
            if let Some(v) = args.get(i + 1) {
                decode_opts.push(v.clone());
                i += 2;
                continue;
            }
        }
    ...
    let nodes = get_packet_details_blocking(file_path, frame_num, &decode_opts)?;
    println!("{}", serde_json::to_string(&nodes).map_err(|e| e.to_string())?);
    Ok(())
}
